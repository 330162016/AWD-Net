...
def build_datasets(root, dataset, size, n_select_bands, scale_ratio):
    ...
    # 输出训练集尺寸信息
    print("\n=== 训练集尺寸信息 ===")
    print(f"train_ref : {train_ref.shape}  # [B, C, H, W]")
    print(f"train_lr  : {train_lr.shape}   # [B, C, H, W]")
    print(f"train_hr  : {train_hr.shape}   # [B, C, H, W]")
    print(f"宽度 H: {train_ref.shape[2]}, 高度 W: {train_ref.shape[3]}, 波段数 C: {train_ref.shape[1]}")
    print("======================\n")
