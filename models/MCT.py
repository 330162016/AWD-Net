...
class MCT(nn.Module):
    def __init__(self,
        ...
        # ---------- 统一确定 U1 / U2 的输入通道，并确保无论 dataset 为何都定义 ----------
        # U1 的输入通道：Pavia(n_bands*3-2) PaviaU(n_bands*3-1) Botswana(n_bands*3-3) Urban(n_bands*3-2) Washington(n_bands*3-1)
        if dataset == 'Pavia':
            self.u1_channel = n_bands * 3 - 2
        elif dataset == 'PaviaU':
            self.u1_channel = n_bands * 3 - 1
        elif dataset == 'Botswana':
            self.u1_channel = n_bands * 3 - 3
        elif dataset == 'Urban':
            self.u1_channel = n_bands * 3 - 2
            ...
            # 给出更明确的提示，避免无定义
            raise ValueError(f"Unknown dataset: {dataset}. Expected one of "
                             f"['Pavia','PaviaU','Botswana','Urban','Washington'].")

        # U2 的输入通道：Pavia(n_bands*2) PaviaU/Botswana/Washington(n_bands*2-2) Urban(n_bands*2)
        if dataset == 'Pavia':
            ...
            self.u2_channel = n_bands * 2 - 2
        # （无需 else，这里 dataset 已在上面校验过）

        # 现在无论哪种 dataset，都定义 U1 和 U2
        self.U1 = nn.Sequential(
            nn.Conv2d(self.u1_channel, n_bands * 2, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
